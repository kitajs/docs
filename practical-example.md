...
fastify({ logger: true }) // [!code focus:3]
  .register(Kita)
  .listen({ port: 1227 });
```
...
Just by creating a file at `src/routes/index.ts` and exporting a `get` function,
you already have a `GET` route registered in your Fastify server.
...
**But wait, there's more!** Kita, along with [Scalar](https://scalar.com/), Kita
generates a graphical interface for you to visualize, edit, and test your API:

<a href="/swagger/practical-example" target="_blank" title="You won't be able to simulate requests">
  Click here to launch a STATIC DEMO of the Scalar Editor
</a>
...
First, you need to run the `kita build` again to re-perform the static analysis
of your code and regenerate the necessary files:
...
Since this is just an practical (non hosted) example, see the interface
generated by Scalar:

<a href="/swagger/practical-example-final" target="_blank" title="You won't be able to simulate requests">
  Click here to launch a STATIC DEMO of the Scalar Editor
</a>
...
<div style="text-align: center">
  <b>With Kita, backend development becomes easy!</b>
</div>
